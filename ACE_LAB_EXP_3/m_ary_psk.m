% 4/8/16-ary PSK Digital Modulation Schemes
% Name: Sachin Chauhan
% Roll# 23SP06007
...
if(mod(bit_len,segment_size)==0)
    ...
    % Phase Modulation Value Array
    if(mod(bit_len,segment_size)==0)
    theta_arr=[];
    for i=1:M %(bit_len/segment_size)
        theta_arr=[theta_arr ((i-1)*2*pi)/M];
    end
    theta_arr
    end
    ...
    for i=1:(bit_len/segment_size)
        for j=1:(M)
            if segment_arr(i)==bin_arr(j)
                ...
                band_pass_sig=[band_pass_sig (cos((2*pi*fc*t)+theta_arr(j)))];
                base_band_in_phase=[base_band_in_phase cos(theta_arr(j))];
                base_band_quad_phase=[base_band_quad_phase sin(theta_arr(j))];
            end
        end
    end
    ...
    title(sprintf('%d - Ary PSK Band-Pass Signal',M));

    subplot(4,1,3);
    stairs(base_band_in_phase);
    ...
    ylabel('Amplitude(Volts)');
    %title(M,'- Array Band-Pass Signal');
    title(sprintf('%d - Ary PSK Base-Band Signal In-Phase Component',M));
    
    subplot(4,1,4);
    stairs(base_band_quad_phase);
    xticks([0:length(base_band_quad_phase)/bit_len:length(base_band_quad_phase)*(1/Ts)]);
    % xticks(0:length(band_pass_sig)/bit_len:length(band_pass_sig)*(1/Ts));
    xticklabels({'0','Ts','2Ts','3Ts','4Ts','5Ts','6Ts','7Ts','8Ts','9Ts','10Ts','11Ts','12Ts','13Ts','14Ts','15Ts','16Ts'})
    % xlim([0 length(base_band_quad_phase)]);
    ylim([-2 2]);
    grid on;
    xlabel('Signal duration(Ts)');
    ylabel('Amplitude(Volts)');
    %title(M,'- Array Band-Pass Signal');
    title(sprintf('%d - Ary PSK Base-Band Signal Quadrature-Phase Component',M));
